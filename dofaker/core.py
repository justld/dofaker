...
class DoFaker:
    ...
    def run(self, input_path:str, dst_face_paths, src_face_paths, output_dir='output'):
        ...
        if input_path.lower().endswith(('jpg', 'jpeg', 'webp', 'png', 'bmp')):
            return self.swap_image(input_path, dst_face_paths, src_face_paths, output_dir)
        else:
            return self.swap_video(input_path, dst_face_paths, src_face_paths, output_dir)


    def swap_video(self, input_video_path, dst_face_paths, src_face_paths, output_dir='output'):
        assert os.path.exists(input_video_path), 'The input video path {} not exist.'
        ...
        if dst_face_paths is not None:
            ...
            assert len(dst_faces) == len(src_faces), 'The detected faces in source images not equal target image faces.'
        ...
        temp_video_path = os.path.join(output_dir, 'temp_{}.mp4'.format(video_name))
        save_video_path = os.path.join(output_dir, '{}.mp4'.format(video_name))
        output_video = cv2.VideoWriter(temp_video_path, four_cc, fps, frame_size)
        ...
        while video.isOpened():
            ...
            if ret:
                if dst_face_paths is not None:
                    swapped_image = self.swap_faces(frame, dst_face_embeddings, src_faces=src_faces)
                else:
                    swapped_image = self.swap_all_faces(frame, src_faces=src_faces)
                i+=1
        ...
        self.add_audio_to_video(input_video_path, temp_video_path, save_video_path)
        os.remove(temp_video_path)
        return save_video_path


    def swap_image(self, image_path, dst_face_paths, src_face_paths, output_dir='output'):
        os.makedirs(output_dir, exist_ok=True)
        src_faces = self.get_faces(src_face_paths)
        if dst_face_paths is not None:
            dst_faces = self.get_faces(dst_face_paths)
        ...
        if dst_face_paths is not None:
            swapped_image = self.swap_faces(image, dst_face_embeddings, src_faces=src_faces)
        else:
    ...
    def add_audio_to_video(self, src_video_path, target_video_path, save_video_path):
    ...
    def get_faces(self, image_paths):
        ...
        for image_path in image_paths:
            ...
            assert len(img_faces) == 1, 'The detected face in image {} must be 1, but got {}, please ensure your image including one face.'.format(image_path, len(img_faces))
            faces += img_faces
        return faces


    def swap_faces(self, image, dst_face_embeddings:np.ndarray, src_faces:list) -> np.ndarray:
        res = image.copy()
        ...
        for i in range(dst_face_embeddings.shape[0]):
            ...
            for idx in index:
                res = self.swapper_model.get(res, image_faces[idx], src_faces[i], paste_back=True)
        return res

    def swap_all_faces(self, image, src_faces:list) -> np.ndarray:
        assert len(src_faces) == 1, 'If replace all faces in source, the number of src face should be 1, but got {}.'.format(len(src_faces))
        res = image.copy()
        image_faces = self.det_model.get(image)
        for image_face in image_faces:
            res = self.swapper_model.get(res, image_face, src_faces[0], paste_back=True)
        return res
